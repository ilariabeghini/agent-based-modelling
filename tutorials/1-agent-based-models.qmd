...
```{python}
from icsspy.abms.sir import SIRModel ## if you opena "icsspy.abms.sir" you see the code in the video (alsi SIRAgent)

model_1_params = {'N': 100, #100 people in this simulated society
 'grid_height': 40,
 'grid_width': 40,
 'infection_rate': 0.3,
 'max_agent_step_size': 1, #the agent can move one cell at a time
...
```
[Importing SIRmodel whose file is in abms, It is common to separate out the code that set up a model from the code you use to fit and run the model.]
...
m1res
```
[At each model step every agent takes a step. We want to plot averything we want at every agent step. Use datacollector to pur together information at every time. We have collected info about the distribution of infected people]
...
print("\nModel 2 Params\n", model_2_params)
```
[model_2 has been stored, so we load it.]
...
```
[Compare the two modules: difference in the transmissibility of the virus. There a lot of uncertainty]
...
[We pull info out of the model and we can use to construct interaction networks.]
...
for i, model in enumerate(models, start=1):
    ...
    G = nx.Graph() # nx. is a package NetworkX
...
```
[we initialize the dictionary; takes the 2 models, and iterate over the 2 modules between agents and graphs.]
...
```
[this models differ for the transmissibility of the infection]
...
[Insights about how the models works/is. And which are the implications of the model. We need to validate this model against the real-world data (robustness-checks)]
...
params = {
    ...
    "grid_height": 40,   # individual variables
    "grid_width": 40,    # individual variables
    "infection_rate": [0.2, 0.4, 0.6, 0.8,],   # how this value is associated with the outcome variables (the same for also the next three)
    "recovery_time_range": [(7,14), (14,24)], # infected for min 7 days
    "max_agent_step_size": [2, 4, 6, 8, 10, 12],
    "n_initial_infections": [1, 5, 10, 15, 20, 25]
}
```
[Ww start by setting our dictionary. What is the sample space that results from the combination of parameters? Agent-past model have not posterior probability distribution. We can figure a model response rate. When we have variables that have lists like these, we tell the model we want a sample space that includes every single combination of the lists.]
...
results = mesa.batch_run(
    model_cls=SIRModel,
    parameters=params,
    iterations=10,  # ten simulations
    max_steps=100,
    number_processes=1, # change n° of processes (increase if powerful machine)
...
results_df.info()
```
[we want to run this model many times: 10 times, and every time is made of 100 steps.]
...
) # these variable are state, not aggregated
...
g.map_dataframe(   # we map the scatterplot
...
```
[load the data from the results dataframe. we have a dataframe that is compatible with seaborn. A plot with 3 columns: we compare the state of the agents. We are dealing with an enourmous amount of data.]
...
```
[apply because it is required by seaborn (?)]
...
```
[4 plots of the distribution of the number/proportion of the population that is infected.]
...
```
[data preparation for doing another sensitivity analysis]
...
[Each agent has a state, continuous opinions that exchanges with others. Large  epsilon, large confidence bound which depends on the opinions of agents.]
...
```{python}
model_params = {'N': 100,
 'epsilon': 0.5, #agents have the same range
 'grid_height': 40,
 'grid_width': 40,
 'max_agent_step_size': 1,
 'n_iterations': 1500
